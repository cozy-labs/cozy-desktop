...
class Merge {
  ...
  async ensureParentExistAsync (side: SideName, doc: *) {
    console.log('Merge#ensureParentExistAsync', doc.path)
  ...
  async resolveConflictAsync (side: SideName, doc: Metadata) {
    console.log('Merge#resolveConflictAsync', doc.path)
    let dst = clone(doc)
  ...
  async addFileAsync (side: SideName, doc: Metadata) {
    console.log('Merge#addFileAsync', doc.path)
    const {path} = doc
    let file
    try {
  ...
  async updateFileAsync (side: SideName, doc: Metadata) {
    console.log('Merge#updateFileAsync', doc.path)
    const {path} = doc
    let file
    try {
      file = await this.pouch.db.get(doc._id)
  ...
  async putFolderAsync (side: SideName, doc: *) {
    console.log('Merge#putFolderAsync', doc.path)
    const {path} = doc
    let folder
    try {
  ...
  async moveFileAsync (side: SideName, doc: Metadata, was: Metadata) {
    console.log('Merge#moveFileAsync', doc.path, was.path)
    const {path} = doc
  ...
  async moveFolderAsync (side: SideName, doc: Metadata, was: Metadata) {
    console.log('Merge#moveFolderAsync', doc.path, was.path)
    const {path} = doc
    if (was.sides && was.sides[side]) {
      let folder
      try {
  ...
  async restoreFileAsync (side: SideName, was: Metadata, doc: Metadata): Promise<*> {
    console.log('Merge#restoreFileAsync', was.path, doc.path)
    const {path} = doc
  ...
  async restoreFolderAsync (side: SideName, was: Metadata, doc: Metadata): Promise<*> {
    console.log('Merge#restoreFolderAsync', was.path, doc.path)
    const {path} = doc
  ...
  async trashFileAsync (side: SideName, was: *, doc: *): Promise<void> {
    console.log('Merge#trashFileAsync', was.path, doc.path)
    const {path} = doc
    let oldMetadata
    try {
  ...
  async trashFolderAsync (side: SideName, was: *, doc: *): Promise<*> {
    console.log('Merge#trashFolderAsync', was.path, doc.path)
    const {path} = doc
  ...
  async deleteFileAsync (side: SideName, doc: Metadata) {
    console.log('Merge#deleteFileAsync', doc.path)
    let file
    try {
      file = await this.pouch.db.get(doc._id)
    } catch (err) {
  ...
  async deleteFolderAsync (side: SideName, doc: Metadata) {
    console.log('Merge#deleteFolderAsync', doc.path)
    let folder
    try {
      folder = await this.pouch.db.get(doc._id)
    } catch (err) {
