  ...
  return Promise.each(scenario.actions, async action => {
    ...
    switch (action.type) {
      ...
      case 'create_file':
        ...
        {
          const parentDir = await helpers.remote.findByPath(
      ...
      case 'update_file':
        ...
        {
          const remoteFile = await helpers.remote.findByPath(`/${action.path}`)
      ...
      case 'trash':
        ...
        {
          const remoteDoc = await helpers.remote.findByPath(`/${action.path}`)
      ...
      case 'restore':
        ...
        {
          const remoteDoc = await helpers.remote.findByPath(
        ...
        debug('- mv', action.src, action.dst)
        {
          ...
          try {
            if (
              ...
              const newParent = await helpers.remote.findByPath(
            ...
            const remoteDoc = await helpers.remote.findByPath(`/${action.src}`)
            ...
            if (err.status === 409) {
              ...
              const remoteOverwriten = await helpers.remote.findByPath(
                `/${action.dst}`
              )
