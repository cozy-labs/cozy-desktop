  ...
  */
  ...
  async syncBatch() /*: Promise<void> */ {
    ...
    try {
      ...
      // no longer stops the whole batch. Independent changes after a failure
      // are still applied; only the dependants of the failed change are
      // skipped for this cycle (tracked via `blockedIds`).
      //
      // `localSeq` is frozen as soon as a failure happens so the failed
      // change re-enters the feed on the next cycle. Independent changes
      // already applied before the failure have advanced `localSeq` past
      // their own seq; those after the failure are applied but do not
      // advance `localSeq` (they'll be re-fetched as no-ops next cycle).
      const blockedIds = new Set()

      for (const changeToApply of this.currentChangesToApply) {
        if (wasSkipped(changeToApply)) {
          if (blockedIds.size === 0) {
            await this.pouch.setLocalSeq(changeToApply.seq)
          }
          this.resolveBlockingCause(changeToApply.id)
          continue
        }

        if (
          graph
            .directPrerequisites(changeToApply)
            .some(d => blockedIds.has(d.id))
        ) {
          blockedIds.add(changeToApply.id)
          continue
        }

        try {
          await this.apply(changeToApply)
          if (blockedIds.size === 0) {
            await this.pouch.setLocalSeq(changeToApply.seq)
          }
          this.resolveBlockingCause(changeToApply.id)
          ...
          const result = await this.handleSyncError(err, changeToApply)
          if (result === 'fatal') return
          if (result === 'blocked') blockedIds.add(changeToApply.id)
          if (
