 ...
 *
 * Makes it easy to include a step in the chain only for some platform.
 */
const only = (platform, step) => platform === process.platform && step

/** The chain of steps for the current platform.
 *
 * Each step can be seen as an actor that communicates with the next one via a
 * buffer. The first step is called the producer and is defined separately.
 */
